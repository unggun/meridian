...
export function evolveThresholds(perfData, config) {
  ...
  // ── 2. maxBinStep ─────────────────────────────────────────────
  // bin_step is discrete (80, 100, 125). If losers cluster at a step and
  // no winners rely on that step or higher, lower the ceiling to exclude it.
  {
    const winnerSteps = winners.map((p) => p.bin_step).filter(isFiniteNum);
    const loserSteps  = losers.map((p) => p.bin_step).filter(isFiniteNum);
    const current     = config.screening.maxBinStep;

    if (loserSteps.length >= 2 && winnerSteps.length >= 1) {
      const minLoserStep = Math.min(...loserSteps);
      const maxWinnerStep = Math.max(...winnerSteps);
      // Only tighten if losers all live at/above a step that no winner uses
      if (minLoserStep > maxWinnerStep && minLoserStep <= current) {
        const newVal = maxWinnerStep;
        if (newVal < current) {
          changes.maxBinStep = newVal;
          rationale.maxBinStep = `Losers all at bin_step>=${minLoserStep}, winners all <=${maxWinnerStep} — tightened from ${current} → ${newVal}`;
        }
      }
    }
  }
