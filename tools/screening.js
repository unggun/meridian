...
export async function getTopCandidates({ limit = 10 } = {}) {
  ...
  // Rescued from the removed OKX enrichment block: uses Meteora's fee_active_tvl_ratio
  // (NOT OKX data), so it survives the OKX removal. Kept gated to non-gmgn to preserve
  // prior behavior — gmgn candidates skip it, exactly as before. To enable under gmgn,
  // drop the `source !== "gmgn"` guard (safe: the isFinite check no-ops on missing data).
